...
$result = sqlcmd -S '(local)\SQL2014' -U 'sa' -P 'Password12!' -database 'master' -Query "SELECT name FROM sys.databases where name = 'ReportServer'" -b -W
...
$result = sqlcmd -S '(local)\SQL2012SP1' -U 'sa' -P 'Password12!' -database 'master' -Query "SELECT name FROM sys.databases where name = 'ReportServer'" -b -W
if ($result -eq 'ReportServer')
{
...
$result = sqlcmd -S '(local)\SQL2008R2SP2' -U 'sa' -P 'Password12!' -database 'master' -Query "SELECT name FROM sys.databases where name = 'ReportServer'" -b -W
if ($result -eq 'ReportServer')
{
