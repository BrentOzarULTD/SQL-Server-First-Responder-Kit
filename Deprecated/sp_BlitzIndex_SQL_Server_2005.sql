...
BEGIN TRY
	BEGIN
		...
		IF(@SkipDisplayMessages = 0) RAISERROR (N'Starting run. sp_BlitzIndex(TM) v2.02 - Jan 30, 2014', 0,1) WITH NOWAIT;
		...
		BEGIN
			...
			IF(@SkipDisplayMessages = 0) RAISERROR (N'Using non-2012 syntax to query sys.dm_db_index_operational_stats',0,1) WITH NOWAIT;
		...
		IF(@SkipDisplayMessages = 0) RAISERROR (N'Adding UQ index on #IndexSanity (object_id,index_id)',0,1) WITH NOWAIT;
		...
		IF(@SkipDisplayMessages = 0) RAISERROR (N'Add computed columns to #IndexSanity to simplify queries.',0,1) WITH NOWAIT;
		...
		IF(@SkipDisplayMessages = 0) RAISERROR (N'Add computed columns to #IndexSanitySize to simplify queries.',0,1) WITH NOWAIT;
		...
		IF(@SkipDisplayMessages = 0) RAISERROR (N'Add computed columns to #missing_index to simplify queries.',0,1) WITH NOWAIT;
...
BEGIN;
	IF @Mode=0 /* DIAGNOSE*/
	BEGIN;
		IF(@SkipDisplayMessages = 0) RAISERROR (N'@Mode=0, we are diagnosing.', 0,1) WITH NOWAIT;
		...
		----------------------------------------
		BEGIN
			IF(@SkipDisplayMessages = 0) RAISERROR (N'check_id 20: >=7 NC indexes on any given table. Yes, 7 is an arbitrary number.', 0,1) WITH NOWAIT;
				INSERT	#BlitzIndexResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
											   secret_columns, index_usage_summary, index_size_summary )
			...
			BEGIN
				IF(@SkipDisplayMessages = 0) RAISERROR (N'Skipping checks on unused indexes (21 and 22) because @Filter=1', 0,1) WITH NOWAIT;
			END
			ELSE /*Otherwise, go ahead and do the checks*/
			BEGIN
				IF(@SkipDisplayMessages = 0) RAISERROR (N'check_id 21: >=5 percent of indexes are unused. Yes, 5 is an arbitrary number.', 0,1) WITH NOWAIT;
				...
				IF(@SkipDisplayMessages = 0) RAISERROR (N'check_id 22: NC indexes with 0 reads. (Borderline)', 0,1) WITH NOWAIT;
				INSERT	#BlitzIndexResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
											   secret_columns, index_usage_summary, index_size_summary )
		...
		BEGIN
			...
			IF(@SkipDisplayMessages = 0) RAISERROR (N'check_id 40: Fillfactor in clustered 90 percent or less', 0,1) WITH NOWAIT;
			INSERT	#BlitzIndexResults ( check_id, index_sanity_id, findings_group, finding, URL, details, index_definition,
										   secret_columns, index_usage_summary, index_size_summary )
					SELECT	40 AS check_id,
							i.index_sanity_id,
			...
			IF(@SkipDisplayMessages = 0) RAISERROR (N'check_id 43: Heaps with forwarded records or deletes', 0,1) WITH NOWAIT;
			WITH	heaps_cte
					  AS ( SELECT	[object_id],
			...
			IF(@SkipDisplayMessages = 0) RAISERROR (N'check_id 44: Heaps with reads or writes.', 0,1) WITH NOWAIT;
			WITH	heaps_cte
